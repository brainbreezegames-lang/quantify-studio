...
// ─── Quantify Knowledge Base (for chat mentor) ──────────────────────────────
...
6. Return COMPLETE HTML — never truncate`
    ...
    if (action === 'chat') {
      ...
      const chatApiKey = rk || process.env.OPENROUTER_API_KEY
      if (!chatApiKey) {
        return res.status(400).json({ error: 'No API key configured.' })
      }
      try {
        ...
        if (!chatResp.ok) {
          ...
          console.error('Quantify chat OpenRouter error:', chatResp.status, errBody)
        ...
        console.error('Quantify chat error:', chatErr)
        return res.status(500).json({ error: 'Chat failed — please try again.' })
      }
    }

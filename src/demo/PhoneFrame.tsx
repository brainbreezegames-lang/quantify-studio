  ...
  return (
    <div className="min-h-screen bg-[#E5E7EB] flex items-center justify-center" style={{ fontFamily: 'Switzer, sans-serif' }}>
      ...
      <div className="hidden md:block">
        ...
        >
          {/* Phone notch/island */}
          <div className="absolute top-[22px] left-1/2 -translate-x-1/2 w-[100px] h-[30px] bg-[#1A1A1A] rounded-full z-20" />
          {/* Screen */}
          <div
          ...
          >
            ...
            {/* Content */}
            <div className="absolute inset-0 overflow-y-auto overflow-x-hidden pt-[44px]">
              {children}
            </div>
            {/* Home indicator */}
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[120px] h-[4px] bg-[#0A0A0A] rounded-full opacity-20 z-10 pointer-events-none" />
          </div>
        </div>
      </div>

      {/* Mobile: full screen */}
      <div className="md:hidden w-full min-h-screen bg-white overflow-y-auto overflow-x-hidden">
        {children}
      </div>
    </div>
  )

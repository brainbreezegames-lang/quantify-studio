...
HIERARCHY: L1=row titles + status badges, L2=secondary text + metadata, L3=timestamps + IDs`,
  ...
  'form-input': `SCREEN ARCHETYPE: Form / Input
ZONES: [app-bar] > [form-groups] > [bottom-actions]
COMPOSITION:
  app-bar: back-arrow + title (e.g. "New Reservation") + optional save icon-btn
  form-groups: 2-3 groups of related fields, each group = section-header + 2-4 fields. Visual divider between groups.
  bottom-actions: primary btn ("Save Order") + secondary btn ("Cancel" or "Discard")
SPACING: app-bar>first-group=16, between-groups=24, between-fields=12, field-label-to-input=4
DATA: realistic field labels (not "Field 1"), proper placeholders ("e.g., Johnson Construction")

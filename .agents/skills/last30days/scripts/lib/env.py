...
def get_config() -> Dict[str, Any]:
    ...
    openrouter_key = os.environ.get('OPENROUTER_API_KEY') or file_env.get('OPENROUTER_API_KEY')
    openai_key = os.environ.get('OPENAI_API_KEY') or file_env.get('OPENAI_API_KEY')
    xai_key = os.environ.get('XAI_API_KEY') or file_env.get('XAI_API_KEY')
    ...
    # Hybrid mode: direct OpenAI for Reddit, OpenRouter for X
    if openai_key and openrouter_key and not xai_key:
        config['XAI_API_KEY'] = openrouter_key
        config['USE_OPENROUTER'] = False  # Don't use OpenRouter for OpenAI (direct key)
        config['USE_OPENROUTER_FOR_XAI'] = True  # Use OpenRouter for xAI
    # OpenRouter-only mode (no direct keys)
    elif openrouter_key and not openai_key and not xai_key:
        config['OPENAI_API_KEY'] = openrouter_key
        config['XAI_API_KEY'] = openrouter_key
        config['USE_OPENROUTER'] = True
        config['USE_OPENROUTER_FOR_XAI'] = True
    else:
        config['USE_OPENROUTER'] = False
        config['USE_OPENROUTER_FOR_XAI'] = False

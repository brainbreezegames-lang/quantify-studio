...
def get_models(
    ...
    # When using OpenRouter for both, just use the pinned models directly
    if config.get("USE_OPENROUTER"):
        result["openai"] = config.get("OPENAI_MODEL_PIN") or "openai/gpt-4o"
        result["xai"] = config.get("XAI_MODEL_PIN") or "x-ai/grok-2-latest"
        return result

    # Hybrid mode: direct OpenAI, OpenRouter for xAI
    use_openrouter_for_xai = config.get("USE_OPENROUTER_FOR_XAI")
    if use_openrouter_for_xai:
        result["xai"] = config.get("XAI_MODEL_PIN") or "x-ai/grok-2-latest"
    ...
    # Only use select_xai_model if NOT using OpenRouter for xAI
    if config.get("XAI_API_KEY") and not use_openrouter_for_xai:
